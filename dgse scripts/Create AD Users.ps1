...
###########################################################
#Additional features/modifications added -Will Urban
  ...
  Import-CSV $newpath | ForEach-Object { 
    ...
    { 
      ...
      { 
        ...
        Else 
        { 
          $lastname = $replace.substring(0,15) 
        } 
        # Create sAMAccountName according to this 'naming convention': 
        # <FirstLetterInitials><LastNameName> for example 
        # wurban
        ...
        { 
          ...
          { 
            ...
            New-ADUser $sam -GivenName $_.GivenName -Surname $_.LastName -DisplayName ($_.LastName + "," + $_.Initials + " " + $_.GivenName) -Office $_.OfficeName -Description $_.Description -EmailAddress $_.Mail -StreetAddress $_.StreetAddress -City $_.City -State $_.State -PostalCode $_.PostalCode -Country $_.Country -UserPrincipalName ($sam + "@" + $dnsroot) -Company $_.Company -Department $_.Department -EmployeeID $_.EmployeeID -Title $_.Title -OfficePhone $_.Phone -AccountPassword $setpass -profilePath $_.ProfilePath -scriptPath $_.ScriptPath -homeDirectory $_.HomeDirectory -homeDrive $_.homeDrive -Enabled $enabled -PasswordNeverExpires $expires 
            Add-ADGroupMember $_.Group $sam  #WORKS!  Adds to the group.
            ...
            #To move to an AD Group:  Add-ADGroupMember pml htehp
        ...
        # Try to create the Home Drive properly
        
        
           # $HomePath = "\\bosarray01.pml.com\Home$"
           # $UserName = $sam
           # $NTDomain = "PML.com"


           # New-Item -Name $UserName -ItemType Directory -Path $HomePath
