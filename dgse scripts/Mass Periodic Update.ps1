...
################################################################################################################################################################
    ...
    Write-Host DEBUG net use L: \\"pmlarray.tml.lab\Administration$" /user:tml\$user "d@t@Gr11"
    net use L: \\"pmlarray.tml.lab\Administration$" /user:tml\$user >null
    ...
    $count2 = Get-Random -minimum 2 -maximum 20
    ReadRandom "L:\" $count2
    Write-host "Read $count2 from Public directory"
    
    $count3 = Get-Random -minimum 1 -maximum 2
...
################################################################################################################################################################
    ...
    #####Using "net use" here after much trial and error. The "> null" piece prevents things from being written to the console, but creates a file on disk called null. 
	Write-Host "`r`n Mounting the L: drive as: " $user
    Write-Host DEBUG net use L: \\"pmlarray.tml.lab\Finance$" /user:tml\$user "d@t@Gr11"
    net use L: \\"pmlarray.tml.lab\Finance$" /user:tml\$user >null
...
#$demoData = "M:\Demo\Data Sets\New Folder structure\Public"
 ...
 foreach ($user in $HumanResources) {
    ...
    #####Using "net use" here after much trial and error. The "> null" piece prevents things from being written to the console, but creates a file on disk called null. 
	Write-Host "`r`n Mounting the L: drive as: " $user
    Write-Host DEBUG net use L: \\"pmlarray.tml.lab\HR$" /user:tml\$user "d@t@Gr11"
    net use L: \\"pmlarray.tml.lab\HR$" /user:tml\$user >null
    #net use L: \\techmkt-01.tml.lab\public /user:tml\$user "d@t@Gr11" >null
    #Sleep for 5 seconds to give the OS time to complete the drive mapping.
	Start-Sleep -s 7
...
foreach ($user in $InformationTechnologyUsers) {
    ...
    #####Using "net use" here after much trial and error. The "> null" piece prevents things from being written to the console, but creates a file on disk called null. 
	Write-Host "`r`n Mounting the L: drive as: " $user
    Write-Host DEBUG net use L: \\"pmlarray.tml.lab\Information_Technology$" /user:tml\$user "d@t@Gr11"
    net use L: \\"pmlarray.tml.lab\Information_Technology$" /user:tml\$user >null
    #net use L: \\techmkt-01.tml.lab\public /user:tml\$user "d@t@Gr11" >null
    #Sleep for 5 seconds to give the OS time to complete the drive mapping.
	Start-Sleep -s 7
...
foreach ($user in $LegalUsers) {
    ...
    #####Using "net use" here after much trial and error. The "> null" piece prevents things from being written to the console, but creates a file on disk called null. 
	Write-Host "`r`n Mounting the L: drive as: " $user
    Write-Host DEBUG net use L: \\"pmlarray.tml.lab\Legal$" /user:tml\$user "d@t@Gr11"
    net use L: \\"pmlarray.tml.lab\Legal$" /user:tml\$user >null
    #net use L: \\techmkt-01.tml.lab\public /user:tml\$user "d@t@Gr11" >null
    #Sleep for 5 seconds to give the OS time to complete the drive mapping.
	Start-Sleep -s 7
...
foreach ($user in $MarketingUsers) {
    ...
    #####Using "net use" here after much trial and error. The "> null" piece prevents things from being written to the console, but creates a file on disk called null. 
	Write-Host "`r`n Mounting the L: drive as: " $user
    Write-Host DEBUG net use L: \\"pmlarray.tml.lab\Marketing$" /user:tml\$user "d@t@Gr11"
    net use L: \\"pmlarray.tml.lab\Marketing$" /user:tml\$user >null
    #net use L: \\techmkt-01.tml.lab\public /user:tml\$user "d@t@Gr11" >null
    #Sleep for 5 seconds to give the OS time to complete the drive mapping.
	Start-Sleep -s 7
...
foreach ($user in $SalesUsers) {
    ...
    #####Using "net use" here after much trial and error. The "> null" piece prevents things from being written to the console, but creates a file on disk called null. 
	Write-Host "`r`n Mounting the L: drive as: " $user
    Write-Host DEBUG net use L: \\"pmlarray.tml.lab\Sales$" /user:tml\$user "d@t@Gr11"
    net use L: \\"pmlarray.tml.lab\Sales$" /user:tml\$user >null
    #net use L: \\techmkt-01.tml.lab\public /user:tml\$user "d@t@Gr11" >null
    #Sleep for 5 seconds to give the OS time to complete the drive mapping.
	Start-Sleep -s 7
...
foreach ($user in $TechnicalServicesUsers) {
    ...
    #####Using "net use" here after much trial and error. The "> null" piece prevents things from being written to the console, but creates a file on disk called null. 
	Write-Host "`r`n Mounting the L: drive as: " $user
    Write-Host DEBUG net use L: \\"pmlarray.tml.lab\TechnicalServices$" /user:tml\$user "d@t@Gr11"
    net use L: \\"pmlarray.tml.lab\TechnicalServices$" /user:tml\$user >null
    #net use L: \\techmkt-01.tml.lab\public /user:tml\$user "d@t@Gr11" >null
    #Sleep for 5 seconds to give the OS time to complete the drive mapping.
	Start-Sleep -s 7
...
foreach ($user in $ExecutiveUsers) {
    ...
    #####Using "net use" here after much trial and error. The "> null" piece prevents things from being written to the console, but creates a file on disk called null. 
	Write-Host "`r`n Mounting the L: drive as: " $user
    Write-Host DEBUG net use L: \\"pmlarray.tml.lab\Executives$" /user:tml\$user "d@t@Gr11"
    net use L: \\"pmlarray.tml.lab\Executives$" /user:tml\$user >null
    #net use L: \\techmkt-01.tml.lab\public /user:tml\$user "d@t@Gr11" >null
    #Sleep for 5 seconds to give the OS time to complete the drive mapping.
	Start-Sleep -s 7
...
################################################################################################################################################################
    ...
    #####Using "net use" here after much trial and error. The "> null" piece prevents things from being written to the console, but creates a file on disk called null. 
	Write-Host "`r`n Mounting the L: drive as: " $user
    Write-Host DEBUG net use L: \\"pmlarray.tml.lab\Public" /user:tml\$user "d@t@Gr11"
    net use L: \\"pmlarray.tml.lab\Public" /user:tml\$user >null
    #net use L: \\techmkt-01.tml.lab\public /user:tml\$user "d@t@Gr11" >null
    #Sleep for 5 seconds to give the OS time to complete the drive mapping.
	Start-Sleep -s 7
